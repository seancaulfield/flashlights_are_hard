/*
 * DualSTAR - Two channel driver based on STAR_off_time version 1.2
 *
 * Changelog
 *
 * 1.0 Initial version
...
#define CAP_THRESHOLD		130	// Value between 1 and 255 corresponding
						// with cap voltage (0 - 1.1v) where we
						// consider it a short press to move to the
						// next mode Not sure the lowest you can go
						// before getting bad readings, but with a
...
int     mode_dir = 0;			//1 or -1. Determined when checking stars. Do we
						...
						// up to a higher mode.
						// NOTE: overwritten by STAR config read
						// routine, so setting this here won't do
						// what you want.
uint8_t mode_cnt = 0;

uint8_t lowbatt_cnt = 0;

uint8_t saved_pwm_lvl = 0;		//Since we can't use the register for the value,
						// as it should be at 0 for most modes
						// (since we typically don't want that
						// circuit on).
...
void set_output(uint8_t pwm_lvl) {
	...
	// WAS:
	//
	// Always set alternate PWM value even if not compiled for dual output as
	// we will use this value throughout the code when trying to see what the
	// current output level is. Setting this wont affect the output when
	// alternate output is disabled.
	//
	// NOW:
	//
...
{	
	...
	while(1) {
		...
		if (low_voltage(ADC_LOW)) {
			...
			if (mode_idx == 0 && saved_pwm_lvl <= modes[mode_idx]) {
				...
				if ((saved_pwm_lvl >> 1) < modes[0]) {
					set_output(modes[0]);
					mode_idx = 0;
				} else {
					set_output(saved_pwm_lvl >> 1);
				}
				// See if we should change the current mode level if we've gone under the current mode.
				if (saved_pwm_lvl < modes[mode_idx]) {
