/* STAR_off_time version 1.5 - RMM Version - Ramp Down 
 *
 * Changelog
 *
 * 1.0 Initial version
 ...
 *			value = (V * 4700 * 255) / ((19100 + 4700) * 1.1)
 *
 *		V_bat (low)  = 2 x 3.0 = 6.0
 *		V_bat (crit) = 2 x 2.8 = 5.6
 *
 *		ADC = ((V_bat - V_diode) * R2   * 255) / ((R1    + R2  ) * V_ref)
 *		    = ((6.0   - .25    ) * 4700 * 255) / ((19100 + 4700) * 1.1) = 153.9 ~= 155
 *		    = ((5.6   - .25    ) * 4700 * 255) / ((19100 + 4700) * 1.1) = 143.2 ~= 145
 *
 */
...
#define MODE_MED			110	// Can comment out to remove mode
//#define MODE_MEDH			39	// Can comment out to remove mode
//#define MODE_MEDHI			39	// Can comment out to remove mode
#define MODE_HIGH			255	// Can comment out to remove mode
//#define MODE_TURBO			255	// Can comment out to remove mode
#define MODE_TURBO_LOW		110	// Level turbo ramps down to if turbo enabled
#define TURBO_TIMEOUT		240 // How many WTD ticks before before dropping down (DIVIDE BY 4)
...
#define ADC_LOW				167	// When do we start ramping
#define ADC_CRIT			150 // When do we shut the light off

#define CAP_THRESHOLD		125  // Value between 1 and 255 corresponding with cap voltage (0 - 1.1v) where we consider it a short press to move to the next mode
...
#endif
#ifdef MODE_MEDH
	modes[mode_cnt++] = MODE_MEDH;
#endif
#ifdef MODE_MEDHI
	modes[mode_cnt++] = MODE_MEDHI;
#endif
#ifdef MODE_HIGH
